...
def copy_sparse_to_frames(source, scene):
    ...
    for item in os.listdir(scene):
        ...
        # print(frame_dir, os.path.isdir(frame_dir), item.startswith('colmap'))
        if os.path.isdir(frame_dir) and item.startswith('colmap'):
            dest_sparse_dir = os.path.join(frame_dir, 'sparse')

            if os.path.exists(dest_sparse_dir):
                print(dest_sparse_dir, os.path.exists(dest_sparse_dir))
                # exit()
                continue
